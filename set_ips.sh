...
# Verificar si se proporcionaron los cuatro argumentos esperados (IP_MEMORIA, IP_CPU, IP_KERNEL, IP_IO)
if [ $# -ne 4 ]; then
  echo "Uso: $0 IP_MEMORIA IP_CPU IP_KERNEL IP_IO"
  exit 1
fi
...
function actualizar_archivos {
  ...
  for archivo in "$directorio"/*.config; do
    ...
    if [ -f "$archivo" ]; then
      ...
      actualizar_valor "$archivo" "IP_IO" "$IP_IO"
    fi
  done
